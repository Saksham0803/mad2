...
def movies():
    ...
    if current_user.is_admin:
        ...
        return jsonify({
            # 'user': current_user.to_dict(),
            'movies': [movie.to_dict() for movie in movies],
            'is_admin' : current_user.is_admin}), 200


@view.route('/movies_user', methods=['GET'])
def movies_user():
    movies = Movie.query.all()
    return jsonify({
        'movies': [movie.to_dict() for movie in movies]
...
@view.route('/show_user/<int:id>', methods=['GET'])
def show_user(id):
    print('hello')
...
def delete_movies(id):
    ...
    if current_user.is_admin:
        ...
        if delete_m:
            ...
            return jsonify({'message': f'Movie {delete_m.title} is deleted'}), 200
        else:
            return jsonify({'message': 'Movie not found'}), 404
    else:
        return jsonify({'message': 'Unauthorized access'}), 403
...
@view.route('/select_theater/<int:id>', methods=['GET'])
def select_theaters(id):
    current_user_id = session.get('user_id')  # get user id from current_user
    ...
    if show:
        ...
        return jsonify({'message': 'There is no show for this movie'}), 200
...
@view.route('/api/my_tickets/<user_id>', methods=['GET'])
def my_tickets(user_id):
    current_user_id = session.get('user_id')  
    print(current_user_id)  # get user id from JWT
    # current_user = RegisteredUser.query.get(current_user_id)  # get user from database
    user = RegisteredUser.query.filter_by(id=current_user_id).first()
    print(user)
    tickets = Ticket.query.filter_by(user=current_user_id).all()
    tickets = [ticket.to_dict() for ticket in tickets]  # assuming you have a to_dict method in your Ticket model

    return jsonify({'user': user.to_dict(), 'tickets': tickets}), 200
...
def book_ticket(id):
    ...
    if request.method == 'POST':
        ...
        if total_seats == 0:
            return jsonify({'message': 'Number of seats not provided'}), 400
        show.available_seats = int(show.available_seats) - int(total_seats)
        new_ticket = Ticket(booked_show=show.id, booked_m=show.screened_m, booked_t=show.t_in, user=current_user_id, movie=show.movie, t=show.t, address_t=show.address_t, total_seats=total_seats, cost=int(show.cost) * int(total_seats), timinig_s=show.datetime)
        
        db.session.add(new_ticket)
        db.session.commit()

        return jsonify({'message': 'Ticket booked successfully', 'ticket': new_ticket.to_dict()}), 201
    else:
        return jsonify({'message': 'GET method not allowed'}), 405
    

# @view.route('/api/book_ticket/<int:id>', methods=['POST'])
# @jwt_required()  # This decorator ensures that a valid JWT is provided
# def book_ticket(id):
#     print(id)
#     show = Show.query.get(int(id))
#     print('start')
...
#         user_id = get_jwt_identity()  # Retrieve the user's ID from the JWT
#         new_ticket = Ticket(booked_show=show.id, booked_m=show.screened_m, booked_t=show.t_in, user=user_id, movie=show.movie, t=show.t, address_t=show.address_t, total_seats=total_seats, cost=int(show.cost) * int(total_seats), timing_s=show.datetime)
...
@view.route('/api/my_movie/<int:id>', methods=['GET'])
@jwt_required()
def my_movies(id):
    print('start1')
    current_user_id = get_jwt_identity()  # get user id from JWT
...
# @view.route("/trigger-celery-job/<int:theater_id>")
