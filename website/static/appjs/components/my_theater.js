...
const MyTheaters = Vue.component('MyTheaters', {
    ...
    methods: {
        ...
        deleteShow(id) {
          axios.delete('/api/delete_shows/' + id, {headers: { 'Authorization': 'Bearer ' + localStorage.getItem('access_token') }})
            .then(response => {
              console.log(response.data);
              // Remove the deleted show from the local data
              this.my_show = this.my_show.filter(show => show.id !== id);
              location.reload();
            })
            .catch(error => {
        ...
        deleteTheater(id) {
          if (confirm('Do you want to delete this Theater?')) {
            this.deleteShow(id);
          axios.delete('/api/delete_theaters/' + id, {headers: { 'Authorization': 'Bearer ' + localStorage.getItem('access_token') }})
            .then(response => {
              console.log(response.data);
              // Refresh the page
              location.reload();
          ...
          <ol class="list-group-fluid list-group-numbered align-middle pt-4" style="width:500px;align-content:center">
            <li class="list-group-item d-flex justify-content-between align-items-start" v-for="theater in my_theater" :key="theater.id">
              <div class="ms-2 me-auto">
                ...
                {{ theater.address }}, <router-link class="card-link" :to="'/update_theatre/' + theater.id">Update</router-link>, <button class="btn btn-danger" @click="deleteTheater(theater.id)">Delete</button>
              </div>
            </li>
          </ol>
        </div>
